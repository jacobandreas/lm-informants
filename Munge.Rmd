...
setwd("G:/My Drive/MIT/Research/lm-informants/")
...
heldout_performance <- read_csv("Exp3/HoldoutEvals.csv") %>% 
...
heldout_combined %>% 
 # mutate(Cost = Cost/Length) %>% 
  group_by(Step,IsLicit,Run,Strategy,N_INIT) %>% 
...
sums <- heldout_combined %>% 
  group_by(Strategy,Step,Run,N_INIT) %>% 
  #crossv_kfold(10) #%>%
  #ungroup() %>% 
 ...
 #         pred = map2( model, test, ~predict( .x, .y, type = "response")) ) %>%
  #select(.id,IsLicit,Strategy,Step,Run) %>%
  #mutate(IsLicit2 = as.factor(if_else(IsLicit == TRUE,"C1","C2"))) %>% 
  #group_by(.id,Strategy,Step,Run) %>%
summarise(auc = roc(IsLicit, exp(-Cost), quiet = T)$auc[1])
...
sums %>% 
  #filter(Run == 0) %>% 
  ggplot(aes(x=Step, y = auc, group = Strategy, color = Strategy, fill = Strategy))+
  geom_smooth()+
  facet_wrap(~N_INIT)+
  theme_bw()
...
a %>% 
  ggplot(aes(x=Step, y = entropy_diff, color = N_Init))+
  geom_point()+
  facet_wrap(~Strategy*Run, nrow =4 )+
  theme_bw()
```


# calculate best possible correlation between model and rating under this featurization

```{r}
dat <- read_csv("out_analysis.csv") 

addins <- read_csv("WordsAndScores.csv") %>% 
...
df_1_model <- lm(Score ~ ., data = df_1)
rsq_1 <- summary(df_1_model)$adj.r.squared

df_2 <- d %>% 
  filter(Source == 2) %>% 
  select(-Word,-Source)

df_2_model <- lm(Score ~ ., data = df_2)
summary(df_2_model)
rsq_2 <- summary(df_2_model)$adj.r.squared
...
df_3_model <- lm(Score ~ ., data = df_3)
summary(df_3_model)
rsq_3 <- summary(df_3_model)$adj.r.squared
...
df_4_model <- lm(Score ~ ., data = df_4)
summary(df_4_model)
rsq_4 <- summary(df_4_model)$adj.r.squared
