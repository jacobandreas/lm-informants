EXP_DIR=results/0621_english
WANDB_PROJECT=0621_english
#TAG=0606_hyperparams_selected
TAG=0621_hyperparams_full
NUM_STEPS=200
NUM_CANDS=50
...
#	python main.py --exp_dir ${EXP_DIR}/warm/batch_prior=${PRIOR}-log_ratio=${LLA} --log_log_alpha_ratio ${LLA} --prior_prob ${PRIOR} --wandb_project $WANDB_PROJECT --num_steps $NUM_STEPS --num_runs 1 --feature_type "english" --eval_humans --strategies unif --feature_type english --num_candidates $NUM_CANDS --tags $TAG 
	python main.py --exp_dir ${EXP_DIR}/warm/batch_prior=${PRIOR}-log_ratio=${LLA} --log_log_alpha_ratio ${LLA} --prior_prob ${PRIOR} --wandb_project $WANDB_PROJECT --num_steps $NUM_STEPS --num_runs 1 --feature_type "english" --eval_humans --strategies unif --feature_type english --num_candidates $NUM_CANDS --tags $TAG --max_updates_observe 1 
	#--verbose 
done
